...
- ~~**Observability / ops**~~ — **DONE.** SyncEngine exposes `lastSyncedAt`/`lastError`/`metrics` + a coarse `status`; `/api/sync` emits a structured JSON log per round to CF observability; `SyncStatus` pill in the bill header. `compaction_health()` (service-role) reports log-growth stats; `compact_stale_bills` logs a structured line per run. **Perf measured:** allocate+settle is ~0.1 ms for a typical bill and ~11 ms for an absurd 100×1000 (100k splits) — sub-frame; cold-start to interactive ~420 ms on prod (TTFB ~20 ms). **Installability:** manifest has 192+512+maskable icons, `display`+`display_override`, theme/bg color, id/start_url; SW with a fetch handler over HTTPS (Lighthouse's PWA category was removed, so audited the criteria directly).
  - **SECURITY FIX (found here):** Supabase default-grants EXECUTE on public functions to anon+authenticated, so maintenance + internal-helper SECURITY DEFINER fns (`compact_bill`, `log_mutation`, `_upsert_split`, stub-makers, …) were directly anon-callable; now revoked from anon/authenticated (migration `…20260626000002`). **Open follow-up:** the client-facing `sync_*` RPCs are likewise reachable by any authenticated principal and don't bind `p_user_id` to `auth.uid()` → a forged `p_user_id` could write as another member of a known bill. Recommend binding `p_user_id = auth.uid()` in all `sync_*`.
