...
describe("EntryGrid", () => {
  ...
  const mockUserId = MOCK_BILL_DATA_COMPLEX.bill_users[0].user_id;
  ...
  it("should call add_bill_item RPC when 'Add Item' is clicked", async () => {
    render(EntryGrid, getProps());
  ...
  it("should call add_bill_contributor RPC when 'Add Contributor' is clicked", async () => {
    render(EntryGrid, getProps());
  ...
  it("should update item name and call upsert", async () => {
    ...
    expect(mockSupabase.from).toHaveBeenCalledWith("bill_items");
    expect(mockOnBillChange).toHaveBeenCalled();
