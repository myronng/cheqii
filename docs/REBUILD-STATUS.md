...
- **5.5 Layout wiring + component migration — IN PROGRESS.**
  ...
  - **REMAINING — the entry editor group (largest chunk):** `bills/[billId]/+page.svelte` (kill the dual-source `$state` copy → read `bills.byId`; discriminated `loading|ready|not_found|error`; per-bill currency via `lib/money`; `allocate`/`settle` from `lib/domain` + `allocationInput`) + `EntryGrid`/`EntryPayments`/`EntrySummary`/`EntrySettings`/`EntryHeader`/`EntryInput`/`EntryName`/`EntrySelect`/`EntryShare` (drop `bind:billData` → call `actions` with `app`+billId; flat splits via `allocationInput`; visibility/invites instead of invite_required/authority). Then 5.8 deletes v1 models + the 3 component tests get rewritten. Remaining ~52 errors are 39 in v1 files deleted by 5.8 (`testMocks`/`bill.svelte`/`user.svelte`) + EntryGrid(4)/EntrySettings(7)/bill-page(2).
