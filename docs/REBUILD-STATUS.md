...
- **5.4 State layer rewrite — IN PROGRESS (the keystone).**
  - **5.4a Convergence reducer — DONE** (commit `64ccbd6`). `src/lib/state/model.ts` (v2 client snapshot types from generated Rows; splits kept **flat** with `allocationInput()` regrouping for the allocate lib) + `src/lib/state/reduce.ts` (`applyBillMutation`/`applyUserMutation` — HLC per-column LWW mirroring the server RPCs, structural fill-once, stub healing, all 15 types). 9 unit tests incl. out-of-order UPDATE_SPLIT→ADD_ITEM heal + per-column survival. 0 type/lint errors.
  - **5.4b State classes — NOT STARTED.** BillState/UserState/AppState + `createMutation` mutation helpers wired to SyncEngine/SyncDB/HLCClock; single source of truth (`bills.byId`), discriminated `loading|ready|not_found|error`, one DI style. **Lands together with 5.5** (components consume the same API) and 5.8 (retire v1 `sync.svelte.ts`/`indexedDb.svelte.ts`) — the class swap + component migration are one coordinated big-bang.
