...
insert into public.cheques (id, name, visibility, is_stub) values
...
insert into public.cheque_users (cheque_id, user_id, role) values
...
insert into public.cheque_people (cheque_id, id, name, sort, is_stub) values
  ('00000000-0000-0000-0000-00000000b001','00000000-0000-0000-0000-0000000000d1','Alice',0,false);
insert into public.cheque_items (id, cheque_id, person_id, name, cost, sort, is_stub) values
...
select pg_temp.read_as('00000000-0000-0000-0000-0000000000a0',
  'select 1 from cheques where id=''00000000-0000-0000-0000-00000000b001''', true,
  'owner reads own private cheque');
select pg_temp.read_as('00000000-0000-0000-0000-0000000000a3',
  'select 1 from cheques where id=''00000000-0000-0000-0000-00000000b001''', false,
  'stranger CANNOT read private cheque');
select pg_temp.read_as('00000000-0000-0000-0000-0000000000a3',
  'select 1 from cheques where id=''00000000-0000-0000-0000-00000000b002''', true,
  'stranger CAN read public_read cheque');
select pg_temp.read_as('00000000-0000-0000-0000-0000000000a0',
  'select 1 from cheques where id=''00000000-0000-0000-0000-00000000b003''', false,
  'owner CANNOT read stub cheque (invisible)');
...
do $$ declare n int; begin
  ...
  set local role authenticated;
  update cheque_items set name='edited-by-editor' where id='00000000-0000-0000-0000-0000000000e1';
...
do $$ declare n int; begin
  ...
  set local role authenticated;
  update cheque_items set name='edited-by-viewer' where id='00000000-0000-0000-0000-0000000000e1';
  get diagnostics n = row_count;
...
do $$ begin
  ...
  set local role authenticated;
  begin
    insert into cheque_items (id, cheque_id, person_id, name, cost, sort)
    values (gen_random_uuid(), '00000000-0000-0000-0000-00000000b001', '00000000-0000-0000-0000-0000000000d1', 'sneaky', 0, 1);
    raise notice 'FAIL: stranger inserted item into private cheque';
  exception when others then
    raise notice 'PASS: stranger insert into private cheque denied (%)', sqlerrm;
  end;
end $$;
