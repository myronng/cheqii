...
- **5.5 Layout wiring + component migration — NEXT (the big-bang).** Wire `+layout.svelte`/`+layout.ts` to `createAppContext` (new v2 AppState); migrate ALL components (entry, main, auth) + `bills/[billId]/+page.svelte` (discriminated `ensureLoaded`) + `MainNewBillButton`/`createBill` onto `src/lib/state/{app.svelte,actions}` + pure allocate/settle + per-bill currency (`lib/money`) + base primitives/ActionBar; rewrite the 3 component test files. This swap (with 5.8) clears the ~52 v1 errors. ~20 .svelte files — the tree won't compile until done; suggested order: layout+main → entry grid → payments/settings/summary. v2 API is ready (`addItem(app, billId, …)`); v1 nested-splits + invite_id/authority → v2 flat + currency/visibility/role.
